...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, output_ch=4, skips=[4], embedder=PositionalEncoder(10,0), embedder_dirs=None, use_viewdirs=False, input_ch=3, input_ch_views=3):
        ...
        #----- self.pts_linears: Pytorch ModuleList. Holdes the MLP architecture from Fig 7 before concatenating with viewing directions
        ...
        # self.pts_linears = nn.Sequential(OrderedDict([nn.Linear(self.W, self.W) for i in range(D)]))
        # self.pts_linears = nn.ModuleList([nn.Linear(self.input_ch,self.W)] + [nn.Linear(self.W + self.input_ch, self.W) if i in skips else nn.Linear(self.W,self.W)
        #                                   for i in range(1,D)])
        self.pts_linears = nn.ModuleList(
            [nn.Linear(self.input_ch,self.W)] + [nn.Linear(self.W, self.W) for i in range(4)] + [nn.Linear(self.W + self.input_ch,self.W)] + [
            nn.Linear(self.W, self.W) for i in range(2)
            ]
        )
        new_skip = [i+1 for i in skips]

        self.pts_linears = nn.ModuleList(
            [nn.Linear(self.input_ch,self.W)] + [nn.Linear(self.W + self.input_ch, self.W) if i in new_skip else nn.Linear(self.W,self.W)
                                                 for i in range(1,D)]
        )
